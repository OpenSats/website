...
export async function fetchGetJSONAuthedBTCPay() {
  try {
    ...
    for(let i=0;i<data.length;i++){
      ...
      if (dataiter[0].cryptoCode == 'XMR' && dataiter[0].totalPaid > 0) {
        numdonationsxmr += 1
        totaldonationsxmr += Number(dataiter[0].totalPaid)
        totaldonationsinfiatxmr += Number(dataiter[0].totalPaid) * Number(dataiter[0].rate)
      }
