...
NDK is one of the most widely used TypeScript libraries for building nostr clients. Production apps such as [Highlighter](https://highlighter.com/), [Lume](https://lume.nu/), and [Flockstr](https://www.flockstr.com/) are built on top of it. By moving the hard parts (relay management, signing adapters, the outbox model, caching, NIP-46 remote signing, zaps, gift-wrapped DMs) into one library, NDK lets app developers focus on their UI and product instead of reimplementing protocol plumbing each time.
