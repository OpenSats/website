...
well as [these articles][eva] might be helpful. Note that various security
researchers and developers encourage the use of [Lockdown Mode][jsr] or a
hardened system like [GrapheneOS][final].
...
Needless to say, but still important to remember, OpenSats does not run a money-printer, therefore can not provide perpetual funding to any grantee. The fact that you have received 1 or 2 (or how many ever) grants in the past is no guarantee that your next application will be approved. We strongly encourage all grantees to always search for other funding sources for their projects besides OpenSats, so that OpenSats does not become a single point of failure. It is inevitable that, at some point, a new grant for a previous grantee will not be approved. That's just the reality of the world of hard money that we're all trying to build.
