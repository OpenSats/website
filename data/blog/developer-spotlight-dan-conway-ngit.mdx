...
Change is here, now time for acceleration. The open source world was never meant to be shackled to one company's server. We're forced to ask: how did we end up with most of the world's open source on Microsoft servers? It began with [Git][git].
