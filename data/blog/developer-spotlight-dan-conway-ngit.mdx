...
We don't even need to stop using GitHub, we just need to also start using tools like ngit/GRASP that nobody owns and that nobody can take away. The open source world was never meant to be living under one company's server. So how did we get into this weird predicament in the first place? Well, it all started with [Git][git].
...
You can find Dan on nostr as [DanConwayDev][dan-npub] and his repositories on [gitworkshop.dev][dan-repos].
