...
characteristics are required to enable high-velocity- and micro-transactions,
streaming payments, and everyday commerce. That’s where Lightning comes in.

As bitcoin++ organizer and Lightning Developer Lisa Neigut explains:
...
recognized as a crucial upgrade for the Lightning Network, enabling users to
...
>always been separate from Lightning but Splicing enables them to be merged
...
This work has required frequent releases to keep pace with LDK's rapid evolution
and ensure compatibility with the latest protocol updates. In parallel, Splicing
messages (including `splice_locked`) have been implemented, along with support
for `Pay-After-Splice` and `Splice-After-Pay`—features that allow channels to
be resized dynamically without interrupting active transactions.
...
`ldk-node`, `ldk-sample`, and `ldk-node-sample`, assessing stability and
readiness for integration. Although several foundational pull requests have been
merged, further work is needed to refine interoperability and finalize message
flows before Splicing is fully production-ready within LDK. Once complete, it
...
has [made important
advancements](https://gitlab.com/lightning-signer/validating-lightning-signer/-/commits/main?ref_type=HEADS)
for both individual users and enterprise deployments. A major milestone came
with the implementation of low-resource signer support, allowing VLS to run
efficiently on remote microcontrollers like
[ESP32](https://en.wikipedia.org/wiki/ESP32) and
[STM32](https://en.wikipedia.org/wiki/STM32). This development opens the door
for lightweight and cost-effective security solutions that can be deployed in
diverse environments, from personal wallets to embedded payment devices. In
parallel, the
[release](https://gitlab.com/lightning-signer/validating-lightning-signer/-/releases)
of Beta 3 introduced multi-signer functionality, ensuring that Lightning node
operators could enforce additional security policies by requiring multiple
validation checks before authorizing transactions. The update also optimized
performance, refined monitoring capabilities, and introduced a Dockerized
deployment model, streamlining integration for developers and businesses seeking
dependable Lightning security.

VLS has also expanded its footprint within the broader Lightning ecosystem
through various integrations. Notably, the project revived
...
usability for developers building on the Lightning Development Kit. VLS also
powers secure transaction signing for [Breez](https://breez.technology/),
[Relai](https://relai.app/), and several other Lightning businesses. Another
major step forward was VLS's integration with
[Greenlight](https://blockstream.com/lightning/greenlight/), a hosted Lightning
node service run by Blockstream. By resolving key security issues in
Greenlight's implementation, VLS has helped strengthen the security model of
remote node hosting, an area of growing interest for enterprises and custodial
service providers. As Christian Decker, Core Engineer at Blockstream put it:
...
Looking ahead, VLS is preparing for its next phase of development, with a focus
on continued enterprise optimization and expanded network compatibility. Work is
already underway to bring support for LND through the
[lndsigner](https://github.com/NYDIG/lndsigner) project, a step toward making
VLS accessible to a broader range of Lightning implementations. Additionally,
planned developments include security features such as Verifiable Secret Sharing
(VSS), Bolt 12 enhancements, and L402 protocol integration. This year, the
project aims to introduce multi-sig capabilities for Lightning transactions,
further strengthening security for high-value and institutional use cases.
...
automation features that enable reproducible network states. Incremental
...
project has achieved a significant milestone by contributing to the integration of LDK with `lnprototest`,
a test suite designed to rigorously validate Lightning protocol behavior. This
work enhances LDK's reliability, ensures adherence to Lightning standards, and
strengthens its overall robustness, giving developers greater confidence when
testing their implementations. A key part of this effort was the launch of the [`ldk-lnprototest`](https://pypi.org/project/ldk-lnprototest/) package on PyPI. This package provides a runner that facilitates the integration of LDK with `lnprototest`, enabling automated and standardized testing. By reducing the risks of protocol inconsistencies and implementation errors—challenges that can significantly impact Lightning's reliability—this tool ensures a more robust and predictable development process for LDK-based implementations like Lampo.
...
>push this forward! —Prakhar Saxena, Lampo Project Lead
...
[BIP39](https://github.com/bitcoin/bips/tree/master/bip-0039) seed words in the
...
updates will also introduce
[BOLT12](https://github.com/lightning/bolts/blob/master/12-offer-encoding.md#bolt-12-negotiation-protocol-for-lightning-payments)
support, refine the plugin system, and expand documentation to encourage broader
...
between implementations, leading to fragmentation and unreliable payment flows.
