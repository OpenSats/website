...
news cycle (or the mempool) might be, Bitcoin marches on, unbothered. Every 10
...
to filter the signal from the noise and constantly execute just the same. Our
...
version is more advanced, with a UX akin to Ride The Lightning.

Clams is privacy-focused, does no logging, has no trackers, and all data stays
...
improved security via a reduced attack surface. The future support for multi-sig
on the Lightning network that VLS will further improve security for users.
Individual users of the Bitcoin Lightning network may weigh the trade-offs of
using VLS differently, some choosing to use it to improve their security, and
some choosing to remain with a hosted node or node on one device for
convenience.
...
Any educational material produced by the project will be made publicly available
and published under a permissive license that is appropriate for the content,
i.e. one of the free licenses listed at
...
[gs]: https://github.com/OpenSats/website/blob/master/docs/grant-selection.md
