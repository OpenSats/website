...
[AssumeUTXO](/topics/assumeutxo) is a feature introduced in Bitcoin Core 26 that
allows for faster node bootstrapping using UTXO snapshots. This project aims to
integrate `AssumeUTXO` functionality into the [Bitcoin Core
App](https://bitcoincore.app/) (BCA), simplifying the process of deploying
Bitcoin nodes on mobile devices, particularly Android. By enabling users to run
fully validating nodes via UTXO snapshots, this integration will improve the
security and decentralization of the Bitcoin network while making it more
accessible. Over the next 6 to 12 months, the project plans to deliver a
