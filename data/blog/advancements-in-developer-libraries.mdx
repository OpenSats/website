...
gained a simpler interface, landed on HPKE-encrypted Oblivious HTTP
transport to protect data without heavy Tor or TLS dependencies, and recently introduced
a session-event log, creating a for standard way for implementors to display transaction
history in apps and throw errors. It now
records more than twenty thousand downloads. The
[`payjoin-cli`](https://crates.io/crates/payjoin-cli) matured into a stable tool
that talks to Bitcoin Core and restarts interrupted async sessions automatically,
giving developers an easy way to test Payjoin
