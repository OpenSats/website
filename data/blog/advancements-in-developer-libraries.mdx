...
want to see in the world, BDK will help you build it." --Steve Myers, BDK
director/core contributor
...
calculations. The
[`locktime`](https://github.com/rust-bitcoin/rust-bitcoin/tree/master/primitives/src/locktime)
module was also
...
secures every bitcoin signature, but Swift developers have long been dealing
with bulky C wrappers because the only audited implementation was written in
C—forcing unsafe pointers, slow builds, and sparse documentation. With
[OpenSats'
support](/blog/bitcoin-grants-july-2024-6th-wave#secp256k1swift),
the [21-DOT-DEV](https://github.com/21-DOT-DEV) collective transformed
[`swift-secp256k1`](https://github.com/21-DOT-DEV/swift-secp256k1) into a
polished, Swift-native toolkit for wallets, Lightning nodes, and [nostr
apps](https://nostrapps.com/).

The [version
0.18.0](https://github.com/21-DOT-DEV/swift-secp256k1/releases/tag/0.18.0)
release introduced
...
signatures into one—improving privacy while saving space—so iOS, macOS, and
Linux apps can enable it with just a few lines of Swift code (instead of dealing
with C wrappers). The API also enforces `non-copyable nonces` and feels as
natural to use as Apple's `CryptoKit`.

A new `DocC`-powered
[website](https://swiftpackageindex.com/21-DOT-DEV/swift-secp256k1/main/documentation/secp256k1)
now builds documentation from the library's source code. Every function and data
type is displayed in plain language, complete with inline explanations and
ready-to-run code samples. Newcomers can open an `Xcode` playground, follow the
step-by-step guide, and compile their first bitcoin transaction in minutes.

With the
[v0.20.0](https://github.com/21-DOT-DEV/swift-secp256k1/releases/tag/0.20.0)
release, the team began shipping a signed
[`XCFramework`](https://github.com/21-DOT-DEV/swift-secp256k1/pull/671) and
[`CocoaPods`](https://github.com/21-DOT-DEV/swift-secp256k1/releases/tag/0.20.0)
spec. CI pipelines that once pulled more than a gigabyte of C toolchains now
only have to fetch a single 6 MB archive, opening the door for closed-source
apps and bandwidth-constrained mobile teams to adopt audited Bitcoin
cryptography.

The [v0.21.0](https://github.com/21-DOT-DEV/swift-secp256k1/releases/tag/0.21.0)
release added a new
[`UInt256`](https://github.com/21-DOT-DEV/swift-secp256k1/pull/561) number type
made for 256-bit math—the size bitcoin keys use. Previously, developers dealt
with large numbers as text strings, a slow and error-prone workaround. With a
real 256-bit integer, the library can run constant-time calculations that block
timing attacks, enforce strict range checks for safer key handling, and pave the
way for future upgrades such as Taproot adjustments—all while exposing a
single, easy-to-use value that feels no more complicated than an ordinary
`UInt64`.

To spread these gains beyond a single library, the team released
[`swift-plugin-tuist`](https://github.com/21-DOT-DEV/swift-plugin-tuist), a
`SwiftPM` plugin that generates reproducible `XCFrameworks` for any package with
a single command—giving other Bitcoin-focused Swift projects a ready-made
build pipeline.

Today, the library resides under the nonprofit-oriented 21.dev organization,
signaling long-term stewardship and clear governance, maintaining alignment with
[OpenSats' mission](/mission) to strengthen open,
transparent infrastructure.

From aggregated `Schnorr` signatures to one-click binary distribution, the
`secp256k1` initiative has turned a niche wrapper into an enterprise-grade
building block. It removes the last major friction points for Swift developers,
making it easier and safer to launch wallets, hardware companions, and privacy
tools that everyday bitcoin users can trust.
...
Payjoin is a protocol for bitcoin transactions that improves on-chain privacy by
breaking the common-input-ownership heuristic while reducing fees through output
consolidation. The [Payjoin Dev Kit](https://github.com/payjoin), a Rust-based
library led by developer [Dan Gould](https://github.com/bitgould), addresses
...
encryption. Mobile wallets and most merchants found that setup impractical. With
[support from
...
([merged into BIP 77](https://github.com/bitcoin/bips/pull/1483)), letting the
sender's wallet and the receiver's wallet exchange transaction details over
channels they already use, allowing both parties to remain offline until it is
time to broadcast the transaction.

The specification quickly turned into working code. The Rust `payjoin`
...
gained a simpler input-output layer, added HPKE-encrypted Oblivious HTTP
transport, and recently introduced a session-event log for easy replay. It now
records more than twenty thousand downloads. The
[`payjoin-cli`](https://crates.io/crates/payjoin-cli) matured into a stable tool
that talks to Bitcoin Core, restarts interrupted sessions automatically, and
ships reproducible Nix builds, giving developers an easy way to test Payjoin
flows. To keep network details private, the team published the [`ohttp-relay
crate`](https://crates.io/crates/ohttp-relay), and community operators such as
[Obscura VPN](https://obscura.net) and [BOBSpaces](https://bobspaces.org) now
run OHTTP relays.

Payjoin Dev Kit now extends beyond Rust. The
[`payjoin-ffi`](https://github.com/LtbLightning/payjoin-ffi) module supplies
bindings for Python, Flutter, and other toolchains, so Android, iOS, and desktop
apps can use the same audited core. Adoption keeps ramping up:
[Bitmask](https://bitmask.app) desktop wallet and
[BDK-CLI](https://github.com/bitcoindevkit/bdk-cli) already send Payjoin V2,
...
Payjoin V2 sending in May 2025. Enterprise-grade roll-outs are still in pilot at
[Galoy's Bria](https://github.com/GaloyMoney/bria) batching engine, at
...
In just over a year, Payjoin Dev Kit has moved from experimental concept to
production standard, complete with relays, directories, language bindings, and
command-line tools. Developers can now add privacy-enhanced payments into
wallets, nodes, and batching services quickly, lowering fees and increasing user
anonymity without the usage of Tor or dedicated servers.
...
the initiative has progressed from a Core Lightning utility to a
...
order, align channel reestablish logic with Eclair, and add sanity checks that
