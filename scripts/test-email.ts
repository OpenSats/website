...
async function testEmail() {
  ...
  console.log(`API Key (first 4 chars): ${process.env.SENDGRID_API_KEY?.substring(0, 4) || 'not set'}`);
  ...
  console.log(`Markdown sanitization applied: ${sanitizedContent.length === sampleReportContent.length ? 'No changes needed' : 'Content sanitized'}`);
  
  // Test with potentially problematic content
  const problematicContent = sampleReportContent + '\n<script>alert("XSS")</script>';
  const sanitizedProblematic = sanitizeMarkdown(problematicContent);
  console.log(`Sanitized problematic content (script removed): ${!sanitizedProblematic.includes('<script>')}`);
  ...
  try {
    ...
    if (process.env.FORCE_REAL_EMAILS === 'true') {
      ...
      console.log('Sending email with retry logic (should succeed on first try)');
