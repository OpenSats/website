...
// BTCPay Server webhook secret for Nostr Fund
const WEBHOOK_SECRET = process.env.BTCPAY_WEBHOOK_SECRET_NOSTR
...
export default async function handler(
  ...
  try {
    ...
    console.log(
      '📨 BTCPay Nostr Fund webhook received with signature:',
    ...
    if (!WEBHOOK_SECRET) {
      console.error('BTCPay Nostr Fund webhook secret not configured')
    ...
    console.log('BTCPay Nostr Fund webhook received:', {
      type: event.type,
    ...
    await processBTCPayWebhook(event, 'The Nostr Fund')

    // Return success response
    res.status(200).json({
      success: true,
      message: 'Nostr Fund webhook processed successfully',
      eventType: event.type,
    })
  } catch (error) {
    console.error('Error processing BTCPay Nostr Fund webhook:', error)
    res.status(500).json({
