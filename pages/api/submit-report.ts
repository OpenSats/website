...
import { sanitizeString, sanitizeEmail, sanitizeMarkdown } from '../../utils/sanitize'
...
async function sendReportEmail(email: string, data: SubmitReportRequest['body'], reportContent: string, commentUrl?: string) {
  ...
  try {
    console.log(`Sending report confirmation email to ${sanitizedEmail} for project "${data.project_name}" (Report #${data.report_number})`);
    
    const result = await sendReportConfirmationEmail(
      sanitizedEmail,
      sanitizeString(data.project_name, 200),
      sanitizeString(data.report_number, 10),
      commentUrl || `https://github.com/OpenSats/reports/issues/${data.issue_number}`,
      reportContent
    );
    
    if (result) {
      console.log(`Email sent successfully to ${sanitizedEmail} for project "${data.project_name}"`);
      return true;
    } else {
      console.error(`Email sending failed to ${sanitizedEmail} for project "${data.project_name}"`);
      return false;
    }
  } catch (error) {
...
async function submitReportViaBot(issueNumber: number, reportContent: string, projectName: string) {
  // Log which token is being used
  const usingBotToken = !!process.env.GH_BOT_TOKEN;
  console.log(`Using ${usingBotToken ? 'bot token' : 'regular token'} for submission`);
  ...
  if (issueNumber === 231) {
    ...
    try {
      ...
      console.log(`Submitting to GitHub as ${usingBotToken ? 'OpenSatsBot' : 'personal account'}`);
      console.log(`Repository: ${process.env.GH_ORG}/${process.env.GH_REPORTS_REPO}, Issue: ${issueNumber}`);
  ...
  if (process.env.NODE_ENV === 'development' && (issueNumber === 123)) {
    ...
    return {
      ...
      url: `https://github.com/${process.env.GH_ORG || 'OpenSats'}/${process.env.GH_REPORTS_REPO || 'reports'}/issues/${issueNumber}#issuecomment-dev-mode`
    };
  }
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        const botResult = await submitReportViaBot(data.issue_number, reportContent, data.project_name);
